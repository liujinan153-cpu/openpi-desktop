...
- **UI v3.1 全面换装（对标 Codex / ChatGPT Desktop）**：深色改石墨近单色（发丝线分层、白底主按钮）；浅色改暖纸色（奶油侧栏 + 暖白主区 + 纯白浮起卡）；工具卡/子代理卡/Todo 条去底色降噪
- **空会话新构图**：幽灵 logo + 大标题上移至视线上部，标题带当前工作区名（如「想在 openpi-desktop 中做点什么？」）；任务卡降为幽灵卡
- **输入区双层结构**：工作区/分支/审批档位/允许清单/上下文接力等 chips 独立成条贴在输入框上方；会话标题改完整胶囊
- **设置改全页**：不再是弹窗——左上「返回应用」、左导航 300px 分组、内容区通栏铺满，支持 Esc 关闭
- **侧栏升级**：底部新增账户条与本会话上下文占用百分比；「项目」tab 新增置顶分区（右键会话即可置顶/取消置顶）
- **全站输入统一体系**：标准输入 32px / 搜索胶囊 / 输入大卡三层级；下拉选择换自绘弹层（暗色主题不再白底蓝条），支持键盘 ↑↓ / Enter / 首字母跳选
- **杂项**：状态栏工具按钮 emoji 换线性图标；品牌/dock/附件字符图标换 lucide；修复记忆弹窗浅色主题边框不可见

## 0.64.0（2026-09-21）

- **模型接入体验重做（P76）**：API 密钥/自定义供应商/本地模型三处统一「⚡ 测试连接」（人话报错：401 密钥无效/超时/连不上，附延迟与模型数）；自定义供应商「⚡ 拉取模型」一键拉取并回填（Ollama 兼容 /api/tags）；会话模型下拉支持「★ 常用/其他」分组 + 管理弹窗勾选展示哪些模型；三步极简引导
- **对话格式对标 ChatGPT（0.63 系列）**：助手回复去气泡卡片/头像，用时折叠条+通栏正文；用户消息圆角气泡；minimap 静音轨道（只标提问）+悬停预览卡
- **界面精修**：侧栏标题对比度根治灰字压灰底；消息操作条浮出式深色胶囊；正文行宽收窄至阅读最优区、留白松绑；支持全界面字体选择与 90%–130% 缩放
- **交付三件套（P71）**：新手安装指引、主进程日志+一键导出脱敏日志包、首次切全自动风险确认

## 0.63.1（2026-09-20）

- **对话排版黑白分明**：正文提亮近白、加粗与小标题纯白（层级拉开，暗色主题；亮色主题不变）；「用时」条收窄为内容宽度；minimap 悬停预览卡收窄
- **minimap 只标提问**：对话轨道每个横杠=一条用户提问，回答内容不再上轨

## 0.63.0（2026-09-20）

- **对话格式对标 ChatGPT**：助手回复去气泡卡片——无背景/边框/头像/AGENT 头，「用时 X分X秒」折叠条当头，正文通栏纯文本；用户消息保留右对齐圆角气泡；minimap 视觉对标（轨道近隐形、单色灰段、视口淡灰蒙层、悬停预览卡）
- **minimap 悬停预览**：鼠标停在对话轨道上弹出该位置消息预览卡（发送者+摘录），点击/拖动跳转不变

## 0.62.0（2026-09-20）

- **常规页（P74）**：设置新增「常规」——主题三档（跟随系统/浅色/深色，跟随系统实时响应系统切换）；字体选择（canvas 探测本机已安装的免费中文字体：微软雅黑/思源黑体/霞鹜文楷/MiSans/HarmonyOS Sans/OPPO Sans，未安装不显示）；字体大小 90%–130% 全界面缩放；全部持久化
- **信息页（P74）**：设置新增「信息」——应用版本（含 Electron/Node 小字）、打开日志、GitHub 问题反馈（自动带版本）、发版日志+检查更新、开发者模式开关（F12/DevTools）；「关于与更新」卡从左导航底部迁入本页（原位置在分类增多后被裁掉的布局问题一并解决）
- **扩展页（P74）**：设置新增「扩展」——内置能力聚合展示（浏览器/电脑控制/生图/联网检索/代码智能/记忆，含能力与权限标注）+ MCP 服务器动态状态行
- **工具与集成页（P73 第三批）**：技能页里历史攒下的六张非技能卡（MCP/联网检索/生图 API/安全审计/上下文管理/Hooks）迁出到独立「工具与集成」页，按集成/安全/高级分节；MCP 卡新增「打开/创建配置」（自动落示例模板），左导航分类列表可滚动
- **安全修复（P73 审查）**：自定义子智能体工具白名单强制过滤（防 bash 卡死主会话 #117）；只读档位下子代理 run_cmd 穿透修复；Windows 保留设备名文件名规避；自定义角色同名静默覆盖改明确报错

## 0.61.0（2026-09-19）

- **子智能体管理页（P73）**：设置新增「子智能体」——内置四角色（探索/写码/测试/审查）开关与说明，自定义角色 CRUD（~/.pi/agent/subagents/，名称/描述/工具白名单/提示词），禁用角色派发自动回退探索者
- **指令页（P73）**：设置新增「指令」——全局 AGENTS.md 编辑器（优先于项目指令生效，保存自动备份 .bak）
- **技能页重排（P73）**：全局级/项目级两级分区+「全部/全局/项目」筛选+搜索；项目级技能只读展示
- **输入区精修**：composer 内框去除（外框已有点亮反馈）；侧栏项目文件夹图标修复（动态图标未重扫导致从不显示）

## 0.60.0（2026-09-19）

- **过程可视化五件套（P72，对标 PI-Desktop）**：① Subagent 协作卡——聊天流内嵌主 Agent→子代理连线卡（模型徽标/实时状态/耗时/真实步骤数，完成折叠失败展开）② 过程时间线——同消息工具卡聚合「处理中 · N 个步骤」折叠条 ③ 会话改动审阅面板——右栏每文件 M/A/D+diffstat，点击看 diff（基线=会话快照）④ 粘贴反馈 toast ⑤ 会话 token 累计指示
- **审阅面板修复（审查）**：基线误取恒回退 HEAD、中文文件名 git 转义全链路坏、取消的子代理状态被覆盖、diff 截断可被压缩单行绕过

## 0.59.1（2026-09-19）

- **交付三件套（P71）**：新手安装指引（docs/INSTALL-GUIDE.md + 应用内入口）；主进程日志落盘（~/.pi/agent/logs/main.log，2MB 轮转）+ 设置页一键导出日志包（自动脱敏，绝不包含 auth.json/sessions）；首次切「全自动」档弹风险确认
- **UI 精修**：全界面圆角/字号/留白 token 化，砍按钮常驻辉光（阴影只留浮层），亮色主题对比度 AA 达标
- **设置页重设计**：左导航布局（分组+搜索+动态大标题），两轮迭代对标 Cherry Studio
- **日志包修复（审查）**：跨盘降级失败、日志正文未脱敏、致命异常静默吞掉、压包黑窗与路径转义
