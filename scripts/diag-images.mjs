...
// 在渲染进程内设置图片并发送（不能从 node 侧直接碰 renderer 的 state）
await ev(`(async()=>{
  state.images = [
    { data: ${JSON.stringify(b64a)}, mimeType: "image/png" },
    { data: ${JSON.stringify(b64b)}, mimeType: "image/png" }
  ];
  document.getElementById("input").value = "请用简洁中文分两段描述这两张截图：每张是什么产品、什么界面、有哪些值得借鉴的UI/功能设计。第一段=第一张图，第二段=第二张图。";
...
for (let i = 0; i < 90; i++) {
  ...
  if (!(await ev("state.streaming")) && got.length > 30) break;
}
console.log("====== 模型转述 ======");
console.log(got);
