...
## P54 验证闭环（✅ 0.44.0）
**目标**：会话收尾前强制自证。
- [x] 系统提示词注入约定：改动代码后必须跑测试/构建并贴证据，跑不了要说明原因
- [x] hooks.json 默认模板（P51 基建）：写类工具后自动 lint（可开关，默认关）
- [x] 验收：e2e-p54 5/5（system 注入断言 + node --check 证据链）

## P55 项目记忆（✅ 0.44.0）
**目标**：跨会话记住项目约定/决策。
- [x] 只读+写两类 memory 工具（存 `<workspace>/.openpi/MEMORY.md`）
- [x] 会话启动注入 MEMORY.md 摘要（超长则截断头部+目录）
