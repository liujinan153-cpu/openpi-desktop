...
> 👉 新用户请先看 [傻瓜安装指引](docs/INSTALL-GUIDE.md)：七步从下载到发出第一条消息，含 SmartScreen 提示处理。
>
> 当前为**公开 Beta**（个人开发者作品，免费使用）。安装包尚未购买 Windows 商业代码签名证书，首次安装可能出现 SmartScreen「未知发布者」提示——点「更多信息 → 仍要运行」即可，请只从本项目 GitHub Releases 下载并核对版本。遇到问题欢迎提 [issue](https://github.com/liujinan153-cpu/openpi-desktop/issues/new?template=bug_report.md)，附上应用内导出的日志包。
