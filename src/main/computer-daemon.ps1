...
# P60 坐坑：混合 DPI 多屏下，非 DPI-aware 进程的 SetCursorPos 与 UIA BoundingRectangle 坐标系不一致（偏差可达数十像素，点偏到窗口外）
# 声明 Per-Monitor V2 后所有坐标统一为物理像素——必须在任何坐标操作前设置
[CUInput]::SetProcessDpiAwarenessContext([IntPtr](-4)) | Out-Null
	...
	try {
		...
		switch ($req.op) {
			...
			"elements" {
				...
				# P60 坐坑 #111：Chromium 系应用 a11y 树需触发后延迟就绪——首个元素无名称时等 2.5s 重试一次
				$anyNamed = $false
				foreach ($e0 in $all) { try { if ([string]$e0.Current.Name) { $anyNamed = $true; break } } catch { } }
				if (-not $anyNamed) { Start-Sleep -Milliseconds 2500; $all = $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition) }
				$out = @()
