...
function createWindow() {
	const win = new BrowserWindow({
		...
		icon: path.join(__dirname, "..", "build", "icon.png"), /* UI v3.1：nativeImage 仅支持 PNG/JPEG（ICO 会空图） */
		webPreferences: {
...
app.whenReady().then(async () => {
	...
	try { const { nativeImage } = require("electron"); const ic = nativeImage.createFromPath(path.join(__dirname, "..", "build", "icon.png")); if (!ic.isEmpty()) win.setIcon(ic); } catch {} /* UI v3.1：图标兜底（PNG） */
