...
OpenPi Desktop ships a built-in Python runtime with every required library preinstalled (python-docx, lxml, Pillow, defusedxml, py7zr, PyMuPDF, reportlab, pypdf, …). Do NOT run `setup.sh` / `env_check.sh` / `pip install` or any environment checks — just use `python3` directly and start working.
