...
## Environment (Ready — Do Not Install)

All dependencies (pypdf, PyMuPDF, reportlab, pdfplumber, pikepdf, Pillow, …) are preinstalled in OpenPi Desktop's built-in Python runtime. Do NOT run `setup.sh` / `env.check` / `env.fix` / `pip install` — everything is already available; just use `python3` directly. If a script needs `$PDF_SKILL_DIR`, source `env_setup/env_check.sh` (it only exports the variable and returns instantly) or resolve the path manually.
...
ALL PDFs must have: Title, Author (default "OpenPi"), Creator, Subject.
...
meta.brand <pdf>             # Add OpenPi metadata
...
> **Dependency**: `pymupdf` is preinstalled in OpenPi Desktop's built-in runtime — no installation needed.
