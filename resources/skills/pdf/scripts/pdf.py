...
def meta_brand(argv: list):
    """Add OpenPi branding metadata to PDF documents."""
    ...
    for input_path in argv:
        ...
        if not quiet:
            ...
            print(f"  Title: {title}")
            print("  Author: OpenPi")
            print("  Creator: OpenPi")
            print("  Producer: OpenPi Desktop")
            if out != input_path:
                print(f"  Output: {out}")
...
def form_render(argv: list):
    ...
    import fitz  # PyMuPDF，预装于内置运行时（替代 pdf2image，免装 poppler）
    ...
    for i, page in enumerate(doc):
        ...
        if max(pix.width, pix.height) > max_dim:
            ...
            pix = page.get_pixmap(matrix=fitz.Matrix(dpi * scale / 72, dpi * scale / 72))
