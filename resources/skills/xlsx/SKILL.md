...
All Python dependencies (openpyxl, Pillow, …) are preinstalled in OpenPi Desktop's built-in runtime. Do NOT run `env_check.sh` or install anything — use `python3` directly and start working. If a script needs `$XLSX_SKILL_DIR`, source `env_setup/env_check.sh` (it only exports the variable and returns instantly).

| Variable | Source | Description |
|----------|--------|-------------|
| `XLSX_SKILL_DIR` | `env_check.sh` 或手动指向技能根目录 | Skill root directory |
| `FONT_DIR` | 系统字体目录 | Windows 交给系统，无需手动设置 |
...
Workbook metadata: `wb.properties.creator = "OpenPi"`
